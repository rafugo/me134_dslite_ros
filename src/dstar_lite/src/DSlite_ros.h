...
#ifndef DSLITE_ROS_CPP
#define DSLITE_ROS_CPP
...
class DSlitePlannerROS : public nav_core::BaseGlobalPlanner {
public:
  
  DSlitePlannerROS (ros::NodeHandle &); //this constructor is may be not needed
  DSlitePlannerROS ();
  DSlitePlannerROS(std::string name, costmap_2d::Costmap2DROS* costmap_ros);
  ...
  vector<int> DSlitePlanner(int startCell, int goalCell);
  ...
  vector <cells> findFreeNeighborCellMod (int CellID);
  ...
  void sortList(vector <cells> OPL);
  float calcKey(int cellID, int startCell, 
                float g_score[], float rhs_score[], float km);
  void updateVertex(cells b1, int startCell, float g_score[], 
                  float rhs_score[], vector <cells> OPL, float km);
  float topKey(vector <cells> OPL);
  void computeShortestPath(int startCell, int goalCell, 
              float g_score[], float rhs_score[], vector <cells> OPL, float km);
